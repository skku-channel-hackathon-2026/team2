...
1. **역할 모델 = 옵션 C.** 모두 고객(새내기)으로 시작하고, 밥약을 하고 싶은 사람만 `/선배로-업그레이드`로 신청해 **제한된 '선배' 역할의 팀원**이 된다.
2. **업그레이드 초대 방식 확정.** 채널톡 앱 API에는 **팀원 초대·이메일 발송 기능이 없다**(§1.5). 그래서 "운영진이 만든 초대 링크 + 앱이 발급한 연결 코드" 조합으로 설계했다. 이메일은 선택지로 둔다(§4.4).
3. **알림을 권한 단위로 분리.** 그룹방 알림(이미 권한 있음)과 새내기 채팅방 알림(`writeUserChatMessage`, 권한 요청 필요)을 다른 단계로 나눴다.
...
한 사람이 새내기(고객 ID)와 선배(팀원 ID)를 **둘 다** 가질 수 있고, `users` 한 행에 두 ID가 연결된다. 이 연결 덕분에 "도감 속 새내기가 선배가 되었다(진화)"를 계산할 수 있다.
...
새내기는 자동으로 계정이 생기고, 선배가 되고 싶은 새내기는 `/선배로-업그레이드` → 운영 승인 → 팀원 가입 → 계정 연결까지 끝낼 수 있다.
...
- [ ] T1-4: 초대 링크로 가입한 팀원이 `/선배시작` + 코드 → `/내정보`에 새내기·선배 두 역할 표시.
...
질문 → 출현 → 선착순 수락 → 만남 완료 → 후기 → **잡기(도감 등록)** 까지 한 바퀴. 알림은 **이미 가진 그룹방 권한만** 쓰고, 새내기는 `/내밥약`에서 상태를 **직접 확인**한다(푸시는 T3).
...
일정·장소가 **수락 시점에 확정**되므로, T2에서는 선배와 새내기가 채팅하지 않아도 만남이 성립한다. 채팅 연결은 T3.

### 5.3 매칭 v1 (단순 규칙)

- **후보:** 연결된 선배, `active`, 질문 분야와 1개 이상 겹침, 새내기 가능 시간과 30분 이상 겹침, 이번 주 상한 남음, 같은 새내기와 진행 중인 볼 없음.
...
- [ ] T2-7: 새내기 취소 → 출현 `cancelled`, 볼 `cancelled`, 선배 주간 사용 시간 복구.
...
새내기가 앱을 열지 않아도 **메신저 푸시로** 진행 상황을 받고, 수락한 선배가 **새내기 채팅방에서 직접 대화**할 수 있게 한다.
...
1. 새내기가 `front` 커맨드를 실행할 때 `context.userChat` ID를 `users.primary_user_chat_id`로 저장.
...
- **진화:** 도감의 새내기(고객 ID)가 T1 연결로 선배(팀원 ID)가 되고 다른 새내기를 잡으면 원래 선배 도감에 "진화" 표시 + 라운지 알림. **T1의 계정 연결이 있어야 계산 가능한 기능**이다.
- **도감 공유:** `/도감` → 라운지 공유 (봇 발송 또는 선배 본인 명의 `writeGroupMessageAsManager`). 공유 동의한 새내기 별명만 노출.
- **재촉·도망:** 선배 재촉 볼당 2회(봇 문구), 7일 무후기 시 도망 (공개 게시 없음).
- **`/답변`:** 내가 도운 질문과 새내기의 자기 답 목록.

**MTF:** 잡기 2회로 Lv2 달성 · 연결된 새내기가 다른 새내기를 잡으면 진화 표시 · 공유 카드에 비동의 새내기 별명이 나오지 않음.
