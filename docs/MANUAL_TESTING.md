...
- 선배 탭: 출현 · 잡은 새내기 · 새내기 도감 · 답변 · 선배 설정 · 운영
...
- 선배 계정을 만드는 전체 경로: 새내기 `내 정보` → 업그레이드 신청 → 선배 `운영`
  → 승인(연결 코드 발급) → 선배 `선배 설정` → 코드 입력 → 분야·가능 시간 저장.
  그 뒤 새내기 `질문하기`로 밥약을 신청하면 선배 `출현`에 도착한다.
...
- 선배로 들어가면 화면이 이미 차 있다. `is_staff = 1`이라 `운영` 탭도 열린다.
  - `출현` 1건 — 지금 수락 가능
  - `잡은 새내기` 7건 — 예정 2 · 후기 대기 2(하나는 재촉 1번 남음) · 잡음 2 ·
    놓침 1
  - `도감` 9명 — 분야 5개에 걸쳐 있고 레벨 2~4, 그중 2명 진화
  - `답변` 2건 — 하나는 공유 미동의라 `익명 후배`로 뜬다
- 이번 주(KST) 약속은 60분어치뿐이라(`weekly_limit_minutes` 600), 시연 중
  즉석에서 만든 요청도 주간 한도에 걸리지 않고 발표자에게 매칭된다.
...
- `출현`(`enc-p4`)의 후보 시간대는 **2026-09-21(월)·09-22(화)** 로 고정돼 있다.
  발표일이 이 주를 넘기면 날짜를 미는 마이그레이션을 새로 하나 추가한다 (적용된
  마이그레이션은 고치지 않는다).
